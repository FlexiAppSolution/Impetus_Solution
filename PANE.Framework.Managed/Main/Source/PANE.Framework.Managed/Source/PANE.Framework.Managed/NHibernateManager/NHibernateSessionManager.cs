namespace PANE.Framework.Managed.NHibernateManager
{
    using NHibernate;
    ...
    public sealed class NHibernateSessionManager
    {
        ...
        private const string SESSION_KEY = "::NHIBERNATE_MANAGED_SESSION_KEY::";
        ...
        private NHibernate.Cfg.Configuration GetConfiguration(string key, string oldKey, DatabaseSource dbSource)
        {
            ...
            if (dbSource == DatabaseSource.Core)
            {
            ...
            else if (!String.IsNullOrEmpty(oldKey))
            {
                ...
                else
                {
                    ...
                    {
                        MfbServiceRef.Mfb mfb = client.GetByCode(oldKey);
                        if (mfb == null)
                        {
                            return null;
                        }
                ...
                {
                    string localFilePath = System.Configuration.ConfigurationManager.AppSettings["LocalNhibernateFile"];
                    configuration.Configure(localFilePath);
        ...
        private void InitSessionFactory()
        {
            ...
            string localFilePath = System.Configuration.ConfigurationManager.AppSettings["LocalNhibernateFile"];
            cfg.Configure(localFilePath);
            this.sessionFactory.Add("", cfg.BuildSessionFactory());
        }
        ...
        private Dictionary<string, ISession> ContextSession
        {
            get
            {
                ...
                {
                    if (HttpContext.Current.Items["::NHIBERNATE_MANAGED_SESSION_KEY::"] == null)
                    {
                        HttpContext.Current.Items["::NHIBERNATE_MANAGED_SESSION_KEY::"] = new Dictionary<string, ISession>();
                    }
                    return (Dictionary<string, ISession>)HttpContext.Current.Items["::NHIBERNATE_MANAGED_SESSION_KEY::"];
                }
                else
                {
                    if (CallContext.GetData("::NHIBERNATE_MANAGED_SESSION_KEY::") == null)
                    {
                        CallContext.SetData("::NHIBERNATE_MANAGED_SESSION_KEY::", new Dictionary<string, ISession>());
                    }
                    return (Dictionary<string, ISession>)CallContext.GetData("::NHIBERNATE_MANAGED_SESSION_KEY::");
            ...
            set
            {
                if (this.IsInWebContext())
                {
                    HttpContext.Current.Items["::NHIBERNATE_MANAGED_SESSION_KEY::"] = value;
                }
                else
                {
                    CallContext.SetData("::NHIBERNATE_MANAGED_SESSION_KEY::", value);
                }
            }
        }
