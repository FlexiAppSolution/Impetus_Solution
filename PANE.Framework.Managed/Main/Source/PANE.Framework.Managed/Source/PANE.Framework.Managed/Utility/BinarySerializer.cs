...
using PANE.Framework.Managed.AuditTrail.Enums;
...
namespace PANE.Framework.Managed.Utility
{
    public class BinarySerializer
    {
        ...
        {
            return PANE.Framework.Managed.Utility.BinarySerializer.SerializeObject(ConvertToList(objectBefore, objectAfter, currentDepth, maxDepth));
        }
        ...
        public static List<TrailItem> ConvertToList(object objectBefore, object objectAfter, int currentDepth, int maxDepth)
        {
            ...
            foreach (PropertyInfo propertyInfo in dataTypeCheck.GetType().GetProperties())
            {
                ...
                else if (propertyType.Equals(typeof(Boolean)) || propertyType.Equals(typeof(Boolean?)))
                {
                    if (!ignoreBeforeText && propertyBefore != null)
                    {
                        strBefore = ((Boolean)propertyBefore).ToString();
                    }
                    if (!ignoreAfterText && propertyAfter != null)
                    {
                        strAfter = ((Boolean)propertyAfter).ToString();
                    }
                }
                else if (propertyType.Equals(typeof(int)) || propertyType.Equals(typeof(int?)))
                {
                    if (!ignoreBeforeText && propertyBefore != null)
                    {
                        strBefore = ((int)propertyBefore).ToString("");
                    }
                    if (!ignoreAfterText && propertyAfter != null)
                    {
                        strAfter = ((int)propertyAfter).ToString("");
                    }
                }
                else if (propertyType.IsEnum || propertyType.IsNullableEnum())
                {
                    if (!ignoreBeforeText && propertyBefore != null)
                    {
                        strBefore = PANE.Framework.Managed.Utility.EnumBinder.SplitAtCapitalLetters(Convert.ToString(propertyBefore));
                    }
                    if (!ignoreAfterText && propertyAfter != null)
                    {
                        strAfter = PANE.Framework.Managed.Utility.EnumBinder.SplitAtCapitalLetters(Convert.ToString(propertyAfter));
                    }
                }
        ...
        private static string GetValue(Type propertyType, object property)
        {
            ...
            else if (propertyType.IsEnum || propertyType.IsNullableEnum())
            {
                if (property != null)
                {
                    return PANE.Framework.Managed.Utility.EnumBinder.SplitAtCapitalLetters(Convert.ToString(property));
                }
            }
            else
            {
    ...
    [DataContract(Namespace = "http://schemas.datacontract.org/2004/07/PANE.Framework.Utility")]
    [Serializable]
